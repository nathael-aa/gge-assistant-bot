...
class ClassementCog(commands.Cog):
    ...
    @app_commands.choices(statistic=[
    ...
    async def classement_statistics(self, interaction: discord.Interaction, statistic: str, player: str):
        ...
        STAT_MAP = {
            "achievements": {"name": t(langue, "cal_stat_achievements", defaut="Points de Succès"), "emoji": "<:season:1523401590936174613>"},
            "plunder": {"name": t(langue, "cal_stat_plunder", defaut="Points de Pillage"), "emoji": "<:loot:1512439015570276553>"},
            "honor": {"name": t(langue, "cal_stat_honor", defaut="Points d'Honneur"), "emoji": "<:honor:1512573860204253214>"},
            "might": {"name": t(langue, "cal_stat_might", defaut="Points de Puissance"), "emoji": "<:pp1:1512438903821570160>"},
            "master": {"name": t(langue, "cal_stat_master", defaut="Puissance de Construction"), "emoji": "<:decoration:1532426465260605630>"},
            "legendary": {"name": t(langue, "cal_stat_legendary", defaut="Niveau Légendaire"), "emoji": "<:lvl:1512571152524906596>"}
        }
        ...
        if not p_info:
            ...
            async with self.bot.session.get(f"https://api.gge-tracker.com/api/v1/players/{urllib.parse.quote(player)}", headers=headers, timeout=5) as r:
                if r.status == 200:
                    data = await r.json()
        ...
        if not p_info:
            return await interaction.followup.send(t(langue, "ev_err_player_not_found", player=player, defaut=f"❌ **{player}** est introuvable. Vérifie l'orthographe du pseudo."))
        ...
        if statistic in ["plunder", "legendary"]:
            ...
            if lvl < 20: tranche = 1
            elif lvl < 30: tranche = 2
            elif lvl < 40: tranche = 3
            elif lvl < 50: tranche = 4
        ...
        if not all_players_raw:
            return await interaction.followup.send(t(langue, "ev_rank_empty", tranche=nom_tranche, ev=stat_info['name'], defaut=f"📭 Aucun classement trouvé pour la **{nom_tranche}**."))
        ...
        if not player_found:
            return await interaction.followup.send(t(langue, "ev_rank_not_in_top", player=player, limit=len(all_players), tranche=nom_tranche, defaut=f"📉 **{player}** n'a pas été trouvé dans le Top {len(all_players)} de la {nom_tranche}."))
        ...
        # --- AJOUT DES COULEURS ---
        if statistic == "achievements": embed_color = discord.Color(0xEADA24)
        elif statistic == "plunder": embed_color = discord.Color(0x522e00)
        elif statistic == "honor": embed_color = discord.Color(0xFFFFFF)
        elif statistic == "might": embed_color = discord.Color(0xa70000)
        elif statistic == "master": embed_color = discord.Color(0xb93132)
        elif statistic == "legendary": embed_color = discord.Color(0xb3ecec)
        ...
        if hasattr(view, 'current_page'): view.current_page = page_cible
        if hasattr(view, 'index'): view.index = page_cible
    ...
    async def classement_joueur(self, interaction: discord.Interaction, evenement: str, player: str):
        ...
        EVENT_MAP = {
            "nomads": {"name": t(langue, "cal_ev_nomad", defaut="Invasion des Nomades"), "emoji": "<:nomads:1512431070719774750>"},
            "foreigners": {"name": t(langue, "cal_ev_realms", defaut="Guerre des Royaumes"), "emoji": "<:war_realms:1512573773658980504>"},
            "samurais": {"name": t(langue, "cal_ev_samurai", defaut="Invasion des Samouraïs"), "emoji": "<:samurai:1512430844935929868>"},
            "bloodcrows": {"name": t(langue, "cal_ev_bloodcrow", defaut="Corbeaux de Sang"), "emoji": "<:bloodcrow:1512430942990368928>"},
            "berimond": {"name": t(langue, "cal_ev_berimond", defaut="Bataille de Bérimond"), "emoji": "<:berimond:1512430901756428390>"}
        }
        ...
        lvl = int(p_info.get("level", p_info.get("lvl", 0)))
        leg_raw = p_info.get("legendaryLevel") or p_info.get("legendary_level") or p_info.get("paragonLevel") or p_info.get("paragon_level") or 0
        leg = int(leg_raw)
        ...
        if evenement == "berimond":
            ...
            if lvl < 70: possible_lids = [1]
            else:
                if leg < 300: possible_lids = [2]
                elif leg < 650: possible_lids = [3]
                elif leg < 950: possible_lids = [4]
                else: possible_lids = [5]
        ...
        if not has_any_data:
            embed_empty = discord.Embed(
                ...
                description=t(langue, "ev_rank_empty", ev=ev_info['name'], defaut=f"Aucun classement trouvé pour **{ev_info['name']}**.\n*L'événement n'est peut-être pas actif ou personne n'a marqué de points.*"),
        ...
        if not player_found:
            if evenement == "berimond":
                ...
                embed_err = discord.Embed(
                    ...
                    description=t(langue, "ev_rank_not_in_top", player=player, limit=1000, tranche=nom_tranche, defaut=f"**{player}** n'a pas été trouvé dans le Top 1000 actuel de la tranche **{nom_tranche}**."),
        ...
        all_players = sorted(all_players_clean, key=lambda x: int(x[0]) if not isinstance(x, dict) else int(x.get("R", 9999)))
        ...
        if evenement == "berimond":
            if found_lid in [1, 3]: embed_color = discord.Color(0x0094ff)
            elif found_lid in [2, 4]: embed_color = discord.Color(0xff0000)
        elif evenement == "nomads": embed_color = discord.Color(0xedcb5a)
        elif evenement == "samurais": embed_color = discord.Color(0xd43c27)
        elif evenement == "bloodcrows": embed_color = discord.Color(0x670111)
        else: embed_color = discord.Color(0x49415d)
    ...
    async def classement_gacha(self, interaction: discord.Interaction, evenement: str, player: str):
        ...
        EVENT_MAP = {
            "flora": {"name": t(langue, "cal_ev_flora", defaut="Piège de la Flore Fatale"), "emoji": "<:FloraToken:1532427755671650465>"},
            "snowglobe": {"name": t(langue, "cal_ev_snowglobe", defaut="La Boule à Neige Enchantée"), "emoji": "<:FrozenCarrot:1532428873768374382>"},
            "hollowmoon": {"name": t(langue, "cal_ev_hollowmoon", defaut="Invocation de la Lune Creuse"), "emoji": "<:Moonegg:1532428876876091573>"},
            "sandfortune": {"name": t(langue, "cal_ev_sandfortune", defaut="Sables de la Fortune"), "emoji": "<:Orange:1532428875424989246>"},
            "banquet": {"name": t(langue, "cal_ev_banquet", defaut="Banquet du Roi"), "emoji": "<:Cake:1532428872639971380>"},
            "midnight": {"name": t(langue, "cal_ev_midnight", defaut="Marché de Minuit"), "emoji": "<:Midnight_key:1532429792413089835>"}
        }
        ev_info = EVENT_MAP.get(evenement, {"name": evenement.capitalize(), "emoji": "<:gacha_currency:1532431673830932612>"})
        ...
        if not all_players_raw:
            return await interaction.followup.send(t(langue, "ev_rank_empty", tranche=nom_tranche, ev=ev_info['name'], defaut=f"📭 Aucun classement trouvé pour la **{nom_tranche}**.\n*{ev_info['name']} n'a peut-être pas encore commencé ou personne n'a marqué de points.*"))
        ...
        if not player_found:
            return await interaction.followup.send(t(langue, "ev_rank_not_in_top", player=player, limit=len(all_players), tranche=nom_tranche, defaut=f"📉 **{player}** n'a pas été trouvé dans le Top {len(all_players)} actuel de la {nom_tranche}."))
            
        mot_classement = t(langue, "ev_word_rank", defaut="Classement")
        titre = f"{ev_info['emoji']} {mot_classement} {ev_info['name']}\n<:icon_points:1512502439339888820> {nom_tranche}"
        
        if evenement == "flora": embed_color = discord.Color(0x81c24a)
        elif evenement == "snowglobe": embed_color = discord.Color(0xffffff)
        elif evenement == "hollowmoon": embed_color = discord.Color(0xff8d00)
        elif evenement == "sandfortune": embed_color = discord.Color(0xffe29c)
        elif evenement == "midnight": embed_color = discord.Color(0x282442)
        else: embed_color = discord.Color.gold()
    ...
    async def classement_realms(self, interaction: discord.Interaction, evenement: str, player: str):
                ...
                if jsonData and str(jsonData.get("return_code")) == "0":
                    ...
                    if l_chunk:
                        ...
                        for p in l_chunk:
                            ...
                            if api_name and (api_name == target_name_exact or api_name == player.lower() or api_name.startswith(f"{player.lower()}_")):
                                player_found = True
                                break
        ...
        if not player_found:
            return await interaction.followup.send(t(langue, "ev_rank_not_in_top", player=player, limit=len(all_players), tranche=nom_tranche, defaut=f"📉 **{player}** n'a pas été trouvé dans le Top {len(all_players)}."))
            
        mot_classement = t(langue, "ev_word_rank", defaut="Classement")
        titre = f"{ev_info['emoji']} {mot_classement} {ev_info['name']}\n<:icon_points:1512502439339888820> {nom_tranche}"
        
        if evenement == "horizon": embed_color = discord.Color(0x4a7160)
        else: embed_color = discord.Color(0xf25500)
    ...
    async def classement_league(self, interaction: discord.Interaction, evenement: str, player: str):
        ...
        EVENT_MAP = {
            "season": {"name": t(langue, "cal_ev_season", defaut="Saison / Festival"), "emoji": "<:season:1523401590936174613>"},
            "league": {"name": t(langue, "cal_ev_league", defaut="Ligue du Royaume"), "emoji": "<:league:1523402089873539192>"}
        }
        ev_info = EVENT_MAP.get(evenement, {"name": evenement.capitalize(), "emoji": "<:leagueicon:1532432050231972030>"})
        ...
        if evenement == "league":
            ...
            if lvl < 70:
                return await interaction.followup.send(t(langue, "ev_err_season_level", defaut="❌ Le classement de saison n'est disponible que pour les joueurs de niveau Légendaire (70+)."))
        ...
        if evenement == "season": embed_color = discord.Color(0xddaadd)
        else: embed_color = discord.Color(0x004d25) # League
    ...
    async def classement_contests(self, interaction: discord.Interaction, evenement: str, player: str):
        ...
        EVENT_MAP = {
            "shapeshifters": {"name": t(langue, "cal_ev_shape", defaut="Les Métamorphes"), "emoji": "<:Shapeshifter:1532432592450752552>"},
            "nobility": {"name": t(langue, "cal_ev_nobility", defaut="Concours de Noblesse"), "emoji": "<:nobility_contest:1532432846461993303>"},
            "woa": {"name": t(langue, "cal_ev_woa", defaut="Guerre des Alliances"), "emoji": "<:woa_points:1512573716259668098>"},
            "patronage": {"name": t(langue, "cal_ev_patronage", defaut="Patronage"), "emoji": "<:patronage:1514704230106140874>"}
        }
        ...
        if evenement == "shapeshifters":
            if lvl < 70: tranche = 1
                ...
                if leg < 300: tranche = 2
                elif leg < 650: tranche = 3
                elif leg < 950: tranche = 4
                else: tranche = 5
        ...
        if evenement == "shapeshifters": embed_color = discord.Color(0x2b211c)
        elif evenement == "nobility": embed_color = discord.Color(0xffbf5b)
        elif evenement == "woa": embed_color = discord.Color(0x512da8)
        else: embed_color = discord.Color.gold()
    ...
    async def classement_alliance(self, interaction: discord.Interaction, categorie: str, alliance_name: str):
        ...
        CAT_MAP = {
            "alliance_honor": {"name": t(langue, "cal_stat_honor", defaut="Points d'Honneur"), "emoji": "<:honor:1512573860204253214>", "color": discord.Color(0xFFFFFF)},
            "alliance_might": {"name": t(langue, "cal_stat_might", defaut="Points de Puissance"), "emoji": "<:pp2:1512571027119538335>", "color": discord.Color(0xbb270d)},
            "alliance_command": {"name": t(langue, "cal_stat_command", defaut="Points de Commandement"), "emoji": "<:Porteurs_de_bouclier:1512574622271279114>", "color": discord.Color(0xafafaf)},
            "alliance_cargo": {"name": t(langue, "cal_stat_cargo", defaut="Points de Fret"), "emoji": "<:pointscargo:1512161268411273429>", "color": discord.Color(0x84ced1)},
            "alliance_nomad": {"name": t(langue, "cal_ev_nomad", defaut="Invasion des Nomades"), "emoji": "<:nomads:1512431070719774750>", "color": discord.Color(0xedcb5a)},
            "alliance_foreigners": {"name": t(langue, "cal_ev_realms", defaut="Guerre des Royaumes"), "emoji": "<:war_realms:1512573773658980504>", "color": discord.Color(0x49415d)},
        ...
        if not all_alliances_raw:
            embed_empty = discord.Embed(
                title="📭 Classement indisponible",
                description=t(langue, "ev_rank_empty", ev=cat_info['name'], defaut=f"Aucun classement trouvé pour **{cat_info['name']}**.\n*L'événement n'est peut-être pas actif ou personne n'a marqué de points.*"),
                color=discord.Color.light_grey()
            )
            await setup_embed_footer(embed_empty, interaction, langue)
            return await interaction.followup.send(embed=embed_empty)
        ...
        if not alliance_found:
            embed_err = discord.Embed(
                ...
                description=t(langue, "ev_rank_not_in_top", player=alliance_name, limit=MAX_RANK, tranche=nom_tranche, defaut=f"L'alliance **{alliance_name}** n'a pas été trouvée dans le Top {MAX_RANK} actuel pour **{cat_info['name']}**."),
                color=discord.Color.orange()
            )
            await setup_embed_footer(embed_err, interaction, langue)
            return await interaction.followup.send(embed=embed_err)
        ...
        all_alliances = sorted(all_alliances_clean, key=lambda x: int(x[0]) if not isinstance(x, dict) else int(x.get("R", 9999)))
