...
class ForteressesCog(commands.GroupCog, group_name="fortress", group_description="Fortress Radar (Sands, Ice, Peaks)"):
    ...
    async def verify_and_send(self, interaction: discord.Interaction, cibles: list, joueur: str, serveur: str, langue: str):
        ...
        etats = {}
        for kid in kids:
            # Passe 1 : Récupérer les libres
            url1 = f"https://api.gge-tracker.com/api/v1/dungeons?page=1&size=50&filterByKid=%5B%22{kid}%22%5D&filterByPlayerName={safe_joueur}&nearPlayerName={safe_joueur}&filterByAttackCooldown=1"
            try:
                async with self.bot.session.get(url1, headers=headers, timeout=10) as r:
                    if r.status == 200:
                        dungeons = (await r.json()).get("dungeons", [])
                        for d in dungeons:
                            etats[f"{kid}_{d['position_x']}_{d['position_y']}"] = {"status": "libre", "cd": 0}
            except: pass
            
            # Passe 2 : Récupérer ceux en cooldown (en feu)
            url2 = f"https://api.gge-tracker.com/api/v1/dungeons?page=1&size=50&filterByKid=%5B%22{kid}%22%5D&filterByPlayerName={safe_joueur}&nearPlayerName={safe_joueur}&filterByAttackCooldown=2"
            try:
                async with self.bot.session.get(url2, headers=headers, timeout=10) as r:
                    if r.status == 200:
                        dungeons = (await r.json()).get("dungeons", [])
                        for d in dungeons:
                            etats[f"{kid}_{d['position_x']}_{d['position_y']}"] = {"status": "feu", "cd": int(d.get("cooldown", 0))}
            except: pass
        ...
        lignes = []
        for c in cibles:
            kxy = f"{c['kid']}_{c['x']}_{c['y']}"
            etat = etats.get(kxy)
            
            if not etat:
                status_txt = t(langue, "fort_verify_unknown", defaut="❓ **Inconnu** *(Trop loin ou bug API)*")
            elif etat["status"] == "libre":
                status_txt = t(langue, "fort_verify_free", defaut="🟢 **Toujours libre !**")
            else:
                mins = max(1, etat["cd"] // 60)
                status_txt = t(langue, "fort_verify_burning", mins=mins, defaut=f"🔥 **En feu** *(Dispo dans {mins} min)*")
                
            lignes.append(f"**{c['royaume']}** ` {c['x']}:{c['y']} ` ➔ {status_txt}")
    ...
    async def relaunch_scan(self, interaction: discord.Interaction, user_id: str, langue: str):
        ...
        if user_id not in data.get("sessions", {}):
            msg_err = t(langue, "fort_err_no_session", defaut="<:error:1512505075220611172> Ta session de scan est terminée ou introuvable. Relance `/fortress scan` pour en démarrer une nouvelle.")
    ...
    async def fetch_cibles(self, joueur: str, kids_to_scan: list, dist_max: int, notified: list, session: aiohttp.ClientSession, serveur: str = "E4K_FR1", langue: str = "fr"):
            ...
            try:
                ...
                if response_data:
                    ...
                    for d in dungeons:
                        ...
                        unk_realm = t(langue, "fort_unknown_realm", kid=kid, defaut=f"Monde {kid}")
                        
                        if uid not in notified:
                            cibles_dispo.append({
                                "uid": uid, "kid": kid, "x": x, "y": y, "dist": dist, "cooldown": 0,
                                "ancien": d.get("player_name", unk_player),
                                "royaume": dict_royaumes.get(kid, unk_realm)
                            })
        ...
        if cibles_dispo:
            ...
            titre = t(langue, "fort_title_avail", defaut="<:667420141394329610:1512573711134490775> CIBLES DISPONIBLES IMMÉDIATEMENT")
                    ...
                    for d in dungeons:
                        ...
                        if uid not in notified:
                            cibles_en_cooldown.append({
                                "uid": uid, "kid": kid, "x": x, "y": y, "dist": dist, "cooldown": cd_secondes,
                                "ancien": d.get("player_name", unk_player),
                                "royaume": dict_royaumes.get(kid, unk_realm)
                            })
            except Exception as e:
        ...
        if cibles_moins_5min:
            ...
            embed_title = t(langue, "fort_title_imminent", defaut="⏳ SURVEILLANCE : Cibles imminentes (< 5 min)")
            ...
            status_label = t(langue, "fort_status_imminent", defaut="🟠 Ouverture dans")
        elif cibles_moins_1h:
            cibles_moins_1h.sort(key=lambda c: c["cooldown"])
            cibles_finales = cibles_moins_1h[:10]
            embed_title = t(langue, "fort_title_anticip", defaut="🕒 ANTICIPATION : Cibles en recharge (< 1 heure)")
    ...
    async def f_scan(
        ...
        st_fluide = t(langue, "fort_status_fluid", defaut="🟢 Fluide")
        st_mod = t(langue, "fort_status_mod", defaut="🟡 Modérée")
        st_elev = t(langue, "fort_status_high", defaut="🔴 Élevée")
        status_charge = st_fluide if nb_sessions_actives < 4 else st_mod if nb_sessions_actives < 8 else st_elev

        embed_conf = discord.Embed(title=t(langue, "fort_setup_title", defaut="📡 Dispositif de Guet Activé"), color=self.clr_activation)
    ...
    async def f_stop(self, interaction: discord.Interaction):
        ...
        if user_id in data["sessions"]:
            ...
            await save_dungeons_async(data)
            await interaction.followup.send(t(langue, "fort_stop_success", defaut="🛑 **Session arrêtée.** Fin des alertes."), ephemeral=False)
        else:
                ...
                try:
                    ...
                    if maintenant > end_dt:
                        ...
                        try:
                            ...
                            msg = t(langue, "fort_session_ended", joueur=joueur, defaut=f"⏰ **Fin de session !** Ton radar pour **{joueur}** s'est éteint.")
                            await user.send(msg)
                        except: pass
                        continue
                except: pass
    ...
    async def f_history(self, interaction: discord.Interaction, player: str):
        ...
        if not player_id:
            try:
                async with self.bot.session.get(f"{base_api}/players/{urllib.parse.quote(player)}", headers=headers, timeout=8) as r:
                    if r.status == 200:
                        p_data = await r.json()
        ...
        stats_desc = t(langue, "fort_hist_stats", 
            ...
            defaut=(
                ...
                f"🗺️ **Royaume favori :** {royaume_prefere} *( {pct_kid:.1f}% des frappes )*\n"
                f"<:fortresses:1512574700839239892> **Jour le plus sanglant :** {jour_actif_str} *( {best_date_count} attaques )*"
            )
        )
        ...
        title = t(langue, "fort_hist_title", p=vrai_nom, defaut=f"🏰 Historique des Pillages : {vrai_nom}")
