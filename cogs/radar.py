...
# 🛠️ On importe nos variables partagées
from utils import BASE_DATA_PATH, joueur_autocomplete, alliance_autocomplete, format_num, PaginationView
...
def save_surveillance(data):
    with open(SURVEILLANCE_FILE, 'w', encoding='utf-8') as f:
...
class RadarSettingsView(discord.ui.View):
    def __init__(self, p_id: str, user_id: str, joueur_name: str):
    ...
    def get_prefs(self):
        ...
        if self.p_id not in data.get("players", {}): return None
        return data["players"][self.p_id]["abonnes"].get(self.user_id, {})

    def toggle_pref(self, pref_key: str):
        data = load_surveillance()
        if self.p_id in data["players"] and self.user_id in data["players"][self.p_id]["abonnes"]:
            current = data["players"][self.p_id]["abonnes"][self.user_id].get(pref_key, False)
            data["players"][self.p_id]["abonnes"][self.user_id][pref_key] = not current
    ...
    def update_buttons_state(self):
        ...
        self.btn_pseudo.style = discord.ButtonStyle.success if prefs.get("pseudo") else discord.ButtonStyle.danger
        self.btn_position.style = discord.ButtonStyle.success if prefs.get("position") else discord.ButtonStyle.danger
        self.btn_alliance.style = discord.ButtonStyle.success if prefs.get("alliance") else discord.ButtonStyle.danger
        self.btn_puissance.style = discord.ButtonStyle.success if prefs.get("puissance") else discord.ButtonStyle.danger
        self.btn_colombe.style = discord.ButtonStyle.success if prefs.get("colombe") else discord.ButtonStyle.danger
...
class RadarAllianceSettingsView(discord.ui.View):
    def __init__(self, a_id: str, user_id: str, alliance_name: str):
        super().__init__(timeout=None)
        self.a_id = a_id
        self.user_id = str(user_id)
    ...
    def get_prefs(self):
        data = load_surveillance()
        if self.a_id not in data.get("alliances", {}): return None
        return data["alliances"][self.a_id]["abonnes"].get(self.user_id, {})

    def toggle_pref(self, pref_key: str):
        data = load_surveillance()
        if self.a_id in data["alliances"] and self.user_id in data["alliances"][self.a_id]["abonnes"]:
            current = data["alliances"][self.a_id]["abonnes"][self.user_id].get(pref_key, False)
            data["alliances"][self.a_id]["abonnes"][self.user_id][pref_key] = not current
            save_surveillance(data)
    ...
    def update_buttons_state(self):
        ...
        self.btn_mouvements.style = discord.ButtonStyle.success if prefs.get("mouvements") else discord.ButtonStyle.danger
        self.btn_rangs.style = discord.ButtonStyle.success if prefs.get("rangs") else discord.ButtonStyle.danger
        self.btn_infos.style = discord.ButtonStyle.success if prefs.get("infos") else discord.ButtonStyle.danger
...
# ==========================================
# 📊 Création du Cog Radar
# ==========================================
class RadarCog(commands.GroupCog, group_name="radar", group_description="🕵️‍♂️ Radar de Guerre personnel"):
    ...
    async def envoyer_alerte_privee(self, abonnes: dict, type_filtre: str, embed: discord.Embed):
        ...
        for user_id, prefs in abonnes.items():
            if prefs.get(type_filtre, False):
                try:
                    embed.set_footer(text=f"Raison : {prefs.get('raison', 'Surveillance')}")
                    user = self.bot.get_user(int(user_id))
                    if not user: user = await self.bot.fetch_user(int(user_id))
    ...
    # ==========================================
    @app_commands.command(name="add", description="🕵️‍♂️ Ajoute un joueur à ton radar personnel (Limite : 25 suivis)")
    ...
    async def s_add(self, interaction: discord.Interaction, joueur: str, raison: str = "Surveillance générale"):
        ...
        # 🚨 VERIFICATION DE LA LIMITE (25 joueurs max par utilisateur)
        ...
        if not already_tracking:
            ...
            if count_tracked >= 25:
                return await interaction.followup.send(f"❌ **Limite de radar atteinte** : Tu suis déjà `{count_tracked}/25` joueurs. Retire un joueur avec `/radar remove` avant d'en ajouter un nouveau.")
        ...
        view = RadarSettingsView(p_id, user_id, joueur)
    ...
    # ==========================================
    alliance_group = app_commands.Group(name="alliance", description="🛡️ Gérer le radar des alliances complètes")

    @alliance_group.command(name="add", description="🛡️ Ajoute une alliance entière à ton radar (Limite : 3 suivis)")
    ...
    async def a_add(self, interaction: discord.Interaction, alliance: str, raison: str = "Surveillance globale"):
        ...
        # 1. On cherche l'ID et les membres de l'alliance
        async with aiohttp.ClientSession() as session:
            try:
                url_search = f"https://api.gge-tracker.com/api/v1/alliances/name/{urllib.parse.quote(alliance)}"
                async with session.get(url_search, headers=headers, timeout=10) as r:
                    if r.status == 200:
        ...
        # 2. Sauvegarde dans le JSON avec limitation
        data = load_surveillance()
        user_id = str(interaction.user.id)

        # 🚨 VERIFICATION DE LA LIMITE (3 alliances max par utilisateur)
        ...
        if not already_tracking:
            ...
            if count_tracked >= 3:
                return await interaction.followup.send(f"❌ **Limite de radar atteinte** : Tu suis déjà `{count_tracked}/3` alliances. Retire une alliance avec `/radar alliance remove` avant d'en ajouter une nouvelle.")
        ...
        view = RadarAllianceSettingsView(a_id, user_id, a_name_real)
        await interaction.followup.send(embed=embed, view=view)
    ...
    async def s_list(self, interaction: discord.Interaction):
        ...
        if mes_alliances:
            embed = discord.Embed(title="🕵️‍♂️ Mon Radar - Alliances", color=discord.Color.dark_blue())
            embed.description = "\n\n".join(mes_alliances)
            embeds.append(embed)
        ...
        if mes_joueurs:
            ...
            for i in range(0, len(mes_joueurs), lignes_par_page):
                ...
                embed = discord.Embed(title="🕵️‍♂️ Mon Radar - Joueurs", color=discord.Color.dark_grey())
                embed.description = "\n\n".join(chunk)
                embeds.append(embed)
    ...
    # ==========================================
    # 🛰️ LE SATELLITE ESPION (Tâche de fond V2)
    # ==========================================
                        ...
                        try:
                            ...
                            async with session.get(url_alli_live, headers=headers, timeout=10) as r_live:
                                if r_live.status == 200:
                                    alli_data = await r_live.json()
                                    if isinstance(alli_data, list) and alli_data: alli_data = alli_data[0]
                                    
                                    new_name = alli_data.get("alliance_name", old_name)
                                    new_members_raw = alli_data.get("players", alli_data.get("members", alli_data.get("playerList", [])))
                                    ...
                                    # 2. Vérif Roster (Entrées/Sorties et Rangs)
                                    ...
                                    # Envoi des alertes correspondantes
                                    if new_leader:
    ...
    @radar_spy_task.error
    async def radar_spy_task_error(self, error):
        logger.error(f"🚨 [RADAR ERREUR FATALE] La tâche s'est arrêtée : {error}")
